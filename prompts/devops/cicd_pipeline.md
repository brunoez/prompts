# PROMPT DE AUDITORIA COMPLETA: SEGURANÇA EM CI/CD, PIPELINES (GITHUB ACTIONS / GITLAB CI) E GERAÇÃO DE RELATÓRIO PDF

## OBJETIVO
Atuar como Engenheiro Principal de DevSecOps e Especialista em Segurança de Pipelines (Yellow Team). Sua missão é realizar uma varredura rigorosa em todas as configurações de **CI/CD e automação** do repositório (GitHub Actions, GitLab CI, CircleCI, Bitbucket Pipelines, Jenkinsfiles, Azure Pipelines). O objetivo é prevenir injeção de comandos em runners (*Script Injection*), vazamento de credenciais na esteira, uso de ações/plugins vulneráveis de terceiros, ausência de isolamento em runners e ataques de cadeia de suprimentos na entrega contínua.
...
2. Identifique os gatilhos (*triggers*), permissões de tokens, segredos referenciados e scripts executados nos jobs.
3. Você DEVE ler e analisar cada workflow linha por linha.

---

## CHECKLIST DE VALIDAÇÃO PRÁTICA (CI/CD & PIPELINE SECURITY)

### 1. Injeção de Expressões e Comandos (Workflow Script Injection)
- [ ] **Uso Inseguro de Contextos em Scripts Inline:** Identifique o uso direto de variáveis de contexto controladas pelo usuário (ex: `${{ github.event.issue.title }}`, `${{ github.head_ref }}`, `${{ github.event.comment.body }}`) dentro de comandos `run:`, permitindo execução arbitrária de código (`eval/bash injection`).
...
### 2. Permissões de Tokens e Princípio do Menor Privilégio
- [ ] **Escopo de `GITHUB_TOKEN` / CI Tokens:** Verifique se os workflows definem permissões mínimas explícitas no nível de arquivo ou job (`permissions: contents: read`), evitando o padrão perigoso de `permissions: write-all`.
- [ ] **Autenticação OIDC vs Chaves Estáticas de Nuvem:** Verifique se as integrações com AWS, GCP ou Azure utilizam autenticação federada via **OpenID Connect (OIDC)** com assunção de Roles temporárias, em vez de chaves de acesso estáticas de longa duração (`AWS_SECRET_ACCESS_KEY`) salvas em segredos de CI.

### 3. Imutabilidade e Pinning de Ações de Terceiros
...
- [ ] **Auditoria de Reputação de Ações:** Identifique Actions mantidas por usuários desconhecidos sem verificação oficial da plataforma.

### 4. Gestão de Segredos, Artefatos e Isolamento de Runners
...
- [ ] **Runners Auto-Hospedados (Self-Hosted Runners):** Verifique se runners auto-hospedados são utilizados em repositórios públicos sem isolamento efêmero (abertos a execução de código por qualquer pull request externo).
...
- **Categoria:** [Script Injection / Token Permissions / Action Pinning / OIDC Hardening / Log Leakage]
- **Problema:** Explicação direta de como um atacante pode comprometer a esteira ou vazar credenciais de produção.
...
a) **Capa:** Título "Relatório de Auditoria de Segurança em CI/CD e Pipelines — <nome do projeto>", data, esteiras mapeadas e postura de DevSecOps.
...
d) **Tabela de Achados Detalhados:** Severidade | Workflow:Linha | Risco Identificado | Correção.
e) **Diretrizes de Pipeline Hardening para o Time.**
f) **Seção Final "ISSUES PARA O GITHUB":** Para cada workflow inseguro, o template completo de issue com o código corrigido.

### REGRAS TÉCNICAS PARA GERAÇÃO DO PDF
- Use um ambiente isolado (ex: `venv` Python com `reportlab` + `matplotlib`).
- Deixe o script gerador salvo no diretório `docs/cicd-audit/`.
